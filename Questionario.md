...
diretamente, ou seja, eles irão criar códigos executáveis para uma plataforma diferente daquela em que o compilador está sendo exerutado. Um exemplo prático disso ocorre quando um desenvolvedor está trabalhando em um computador com arquitetura x86 como um computador comum, mas deseja compilar um programa para ser executado em um dispositivo móvel com arquitetura ARM, dessa forma ele faria uma compilação cruzada para criar o binário compatível com a plataforma de destino.
...
O NVIC é um componente projetado para gerenciar e responder a eventos de interrupção, independentemente de sua fonte. Ele oferece um método eficaz para lidar com essas situações de maneira ordenada e com prioridade. Desta forma prioriza interrupções de alta prioridade para garantir que eventos críticos sejam atendidos primeiro, assegurando uma resposta rápida. Além disso, é capaz de lidar com interrupções que ocorrem durante o tratamento de outra interrupção, gerenciando em situações de múltiplas interrupções. O NVIC é configurável e pode ser adaptado para atender às necessidades específicas da aplicação. Em sistemas de tempo real, o NVIC garante o tratamento eficiente e oportuno das interrupções.

### (j) Em modo de execução normal, o Cortex-M pode fazer uma chamada de função usando a instrução **BL**, que muda o **PC** para o endereço de destino e salva o ponto de execução atual no registador **LR**. Ao final da função, é possível recuperar esse contexto usando uma instrução **BX LR**, por exemplo, que atualiza o **PC** para o ponto anterior. No entanto, quando acontece uma interrupção, o **LR** é preenchido com um valor completamente  diferente,  chamado  de  **EXC_RETURN**.  Explique  o  funcionamento  desse  mecanismo  e especifique como o **Cortex-M** consegue fazer o retorno da interrupção. 
Quando uma função é chamada no modo de execução normal, a instrução BL é empregada para atualizar o PC para o endereço da função chamada, enquanto o endereço da próxima instrução é armazenado no registrador LR, permitindo que a função retorne ao ponto de chamada após a conclusão. Entretanto, em caso de interrupção, o Cortex M automaticamente preserva o contexto atual na pilha e insere o valor EXC_RETURN no LR para especificar como o processador deve retornar após o tratamento da interrupção. O EXC_RETURN é um valor especial que determina se o processador deve retornar ao modo não privilegiado ou privilegiado, qual pilha deve ser usada e se o estado anterior deve ser restaurado. Ao concluir o tratamento da interrupção, o valor EXC_RETURN deve ser utilizado para restaurar o contexto. Se o bit 0 for 0, o processador volta ao modo de usuário; se for 1, ele retorna ao modo privilegiado. O bit 3 indica se o PSP ou MSP deve ser utilizado para restaurar a pilha, enquanto outros bits controlam diversas configurações adicionais.

### (k) Qual  a  diferença  no  salvamento  de  contexto,  durante  a  chegada  de  uma  interrupção,  entre  os processadores Cortex-M3 e Cortex M4F (com ponto flutuante)? Descreva em termos de tempo e também de uso da pilha. Explique também o que é ***lazy stack*** e como ele é configurado. 
Durante a ocorrência de uma interrupção, os Cortex-M3 e Cortex-M4F apresentam distinções no procedimento de preservação de contexto. Enquanto o Cortex-M3 não possui suporte integrado para o contexto de ponto flutuante, exigindo que o software salve manualmente o contexto do processador empilhando os registradores no espaço de memória designado. Por outro lado, o Cortex-M4F dispõe de hardware especializado para tratar o contexto, automatizando a preservação dos registradores de ponto flutuante em uma área dedicada da memória conhecida como "stack de ponto flutuante". Além disso, ele adota a técnica denominada "lazy stack", que adia o salvamento de parte do contexto na pilha principal até que seja realmente necessário, sendo configurada através de registros específicos do NVIC. Essa técnica permite a conservação imediata apenas dos registradores essenciais na pilha principal, resultando em tempos de resposta reduzidos.
